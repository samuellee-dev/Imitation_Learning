# 모방학습 실험 — Diffusion Policy vs ACT, 태스크에 따라 승자가 바뀐다

LeRobot으로 Diffusion Policy와 ACT를 직접 학습시키고(사전학습 모델 사용 아님),
공개 데이터셋 두 종류(pusht, aloha 삽입)에서 두 모델을 비교했습니다.
벤치마크 숫자를 재현하는 데 그치지 않고, **기본 설정값이 정말 최선인지**,
**"이 모델이 더 낫다"는 결론이 태스크가 바뀌어도 유지되는지**를 직접 검증했습니다.

> **3줄 요약**
> 1. 기본 설정을 그대로 쓰면 두 모델의 성능 차이가 왜곡된다 (실행 조건, 체크포인트 선택 모두 영향).
...
재학습 없이 실행 설정만 바꿔도 성능이 크게 달라졌고, 태스크를 바꾸면 우세한 모델 자체가 뒤집혔습니다.
