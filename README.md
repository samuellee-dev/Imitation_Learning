...
4. **두 모델은 서로 다른 지점에 있었다.**
   Diffusion Policy는 이미 과적합 구간에 들어서 정체됐지만, ACT는 100,000
   스텝에서도 계속 개선되는 중이었습니다. 정확도 비교(64~68% vs 29.5%)는
   같은 학습량에서의 결과일 뿐, 각 모델이 최대 성능에 도달했는지는 다른 문제입니다.
...
- ACT는 100,000 스텝 시점에도 계속 개선되는 추세였지만, 그 이상 학습시켜보지는
  않았습니다. ACT의 실제 최대 성능은 확인하지 못했습니다.
